...
const Navbar = () => {
  ...
  useGSAP(() => {
    ...
    tl.current = gsap
      ...
      .to(
        linksRef.current,
        { autoAlpha: 1, x: 0, stagger: 0.1, duration: 1, ease: "power2.inOut" },
        "<"
      )
      .to(
        contactRef.current,
  ...
  return (
    <>
      <nav
        ref={navRef}
        className="fixed z-50 flex flex-col justify-between w-full h-full px-10 bg-black text-white/80 py-28 gap-y-10 md:w-1/2 md:left-1/2"
      >
        <div className="flex flex-col text-5xl gap-y-2 2xl:text-8xl">
      ...
      <div
        className="menu-button fixed z-50 flex flex-col items-center justify-center gap-1 transition-all duration-300 bg-black rounded-full cursor-pointer w-14 h-14 md:w-20 md:h-20 top-4 right-4 lg:right-10"
        onClick={toggleMenu}
      >
        <span
          ref={topLineRef}
